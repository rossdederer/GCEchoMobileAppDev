...
module wijmo.gauge {
    ...
    export class Gauge extends Control {
        ...
        constructor(element: any, options?) {
            ...
            this.addEventListener(this.hostElement, 'mousemove', function (e: MouseEvent) {
            ...
            if ('ontouchstart' in window) {
                this.addEventListener(this.hostElement, 'touchstart', function (e: PointerEvent) {
                    self.focus();
                    if (!e.defaultPrevented && !self.isReadOnly && self._applyMouseValue(e, true)) {
                        e.preventDefault();
                    }
                });
                this.addEventListener(this.hostElement, 'touchmove', function (e: PointerEvent) {
                    if (!e.defaultPrevented && !self.isReadOnly && self._applyMouseValue(e, true)) {
                        e.preventDefault();
                    }
                });
            ...
            this.addEventListener(this.hostElement, evt, function (e: MouseWheelEvent) {
                if (!e.defaultPrevented && !self.isReadOnly && self.containsFocus() && self.value != null && self.hitTest(e)) {
                    ...
                    self.value = clamp(self.value + (self.step || 1) * step, self.min, self.max);
                    e.preventDefault();
                }
            });
