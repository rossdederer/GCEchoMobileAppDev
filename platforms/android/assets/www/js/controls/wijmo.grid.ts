...
module wijmo.grid {
    ...
    export class FlexGrid extends Control {
        ...
        /*private*/ _rtl = false; // accessible to CellFactory
...
module wijmo.grid {
    ...
    export class CellRangeEventArgs extends CancelEventArgs {
         ...
         */
        constructor(panel: GridPanel, rng: CellRange, data?: string) {
            super();
...
module wijmo.grid {
    ...
    export class CellFactory {
         ...
         */
        public updateCell(panel: GridPanel, r: number, c: number, cell: HTMLElement, rng?: CellRange, updateContent?: boolean) {
            var g = panel.grid,
                ct = panel.cellType,
                rows = panel.rows,
                cols = panel.columns,
            ...
            if (updateContent != false) {
                ...
                if (ct == CellType.Cell && c == g.columns.firstVisibleIndex &&
                ...
                } else if (ct == CellType.Cell && col.dataType == DataType.Boolean && (!gr || isBoolean(data))) { // TFS 122709
                    ...
                    if (!col.dataMap && !col.mask) { // TFS 123276
    ...
    export class GroupRow extends Row {
        ...
        getGroupHeader(): string {
            var
                ...
                group = <wijmo.collections.CollectionViewGroup>tryCast(this.dataItem, wijmo.collections.CollectionViewGroup);
            if (group && fmt) {
    ...
    export class HitTestInfo {
        ...
        constructor(grid: any, pt: any) {
            ...
            if (this._p == null && pt.x >= 0 && pt.y >= 0 &&
                sz && pt.x <= sz.width + hdrWid && pt.y <= sz.height + hdrHei) {
...
module wijmo.grid {
    ...
    export class _MouseHandler {
        ...
        constructor(grid: FlexGrid) {
            ...
            grid.addEventListener(e, 'mousedown', function (e: MouseEvent) {
        ...
        private _mouseDown(e: MouseEvent) {
            ...
            if (e.target != document.activeElement && !g.containsFocus()) {
                ...
                var cell = <HTMLElement>e.target;
                while (cell instanceof Node && !hasClass(cell, 'wj-cell')) {
                    cell = <HTMLElement>cell.parentNode;
                }

                // give focus to cell, panel, or grid
                if (ht.panel == g.cells && cell instanceof HTMLElement) {
                    var fc = <HTMLElement>cell.firstElementChild;
                    if (fc instanceof HTMLElement) {
                        if (fc.tabIndex < 0) { // make sure element can get focus (needed in Chrome, TFS 131286)
                            fc.tabIndex = 0;
                        }
                        fc.focus();
                    } else {
                    ...
                    if (p.tabIndex < 0) { // make sure panel can get focus (needed in Chrome, TFS 131286)
                        p.tabIndex = 0;
                    }
                    p.focus();
                }
        ...
        private _mouseUp(e: MouseEvent) {
            ...
            if (htd && htd.cellType == CellType.TopLeft && !this._szArgs) {
                var g = this._g,
                    ht = g.hitTest(e);
