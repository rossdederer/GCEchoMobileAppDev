...
module wijmo.chart {
    ...
    export class FlexPie extends FlexChartBase {
        ...
        private _select(pointIndex: number, animate: boolean= false) {
            ...
            // need invalidate in this case
            if (this.selectedItemOffset > 0) {
                if (this.isAnimated) {
                    if (this.selectedItemPosition == Position.None) {
                        this.invalidate();
                    }
                } else {
                    this.invalidate();
        ...
        private _layoutMultiple(rect: Rect, size: Size, engine: IRenderEngine) {
            ...
            for (var icol = 0; icol < ncols; icol++) {
                ...
                for (var i = 0; i < ad.axes.length; i++) {
                    ...
                    var ah = ax._getHeight(engine);// .GetSize(GetItems(render, ax), false);
                    if (ah > mxsz.height)
                        ah = mxsz.height;
            ...
            for (var irow = 0; irow < nrows; irow++) {
                ...
                for (var i = 0; i < ad.axes.length; i++) {
                    var ax = <Axis>ad.axes[i];
                    var szy = new Size(mysz.width, ax._getHeight(engine));
                            ...
                            if (isNear) {
                                ...
                                if (showLabel) {
                                    ...
                                    if (labelAngle != 0) {
                                        ...
                                        this._renderLabel(engine, val, sval, lpt, lalign /*1*/, 2, lblClass);
                                    }
                                }
            ...
            if (this.isCandle) {
                ...
                if (_DataInfo.isValid(hi) && _DataInfo.isValid(lo)) {
                    ...
                    if (rotated) {
                        ...
                        area = new _RectArea(new Rect(Math.min(hi, lo), x - 0.5 * w, Math.abs(hi - lo), w));
                    } else {
...
module wijmo.chart {
    ...
    export class _VolumeHelper {
        ...
        private _getXVolume(x: number): number {
            ...
            if (this._hasXs) {
                i = this._xVals.indexOf(x);
                if (this._xDataType === DataType.Date && i === -1) {
                    i = this._xVals.indexOf(new Date(x).setHours(0, 0, 0, 0));
                }
            }
