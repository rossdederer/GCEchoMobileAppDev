...
module wijmo {
    ...
    export class Globalize {
         ...
         */
        static formatNumber(value: number, format: string, trim = false): string {
            asNumber(value);
            asString(format);

            var result,
                m = format ? format.match(/([a-z])(\d*)(,*)(.*)/i) : null,
                nf = wijmo.culture.Globalize.numberFormat,
            ...
            for (var i = 0; i < fparts.length && vparts; i++) {
                ...
                switch (fparts[i]) {
                    ...
                    case 'g':
                        ...
                        break;
                    case 'K': // time zone information, not supported
                        // TODO!!!!
                        break;
                    case cal['/']: // localized separators (TFS 131320)
                    case cal[':']:
                        break;
                    default:
                        offset++;
        ...
        // parse a date forma string into its parts
...
module wijmo {
     ...
     */
    export function showPopup(popup: HTMLElement, ref: any = null, above = false, fadeIn = false) {
        ...
        if (ref instanceof MouseEvent) {
            ...
            rc = ref;
        } else if (ref == null) { // no reference: center on window
...
module wijmo {
    ...
    export class _MaskProvider {
        ...
        _input() {
            setTimeout(() => { // timeOut is **important** for Windows Phone/Android/Safari
                this._valueChanged(); 
            });
        }
        ...
        _valueChanged() {
            ...
            if (input) {
                ...
                var start = input.selectionStart,
                    oldChar = (start > 0) ? input.value[start - 1] : '';
