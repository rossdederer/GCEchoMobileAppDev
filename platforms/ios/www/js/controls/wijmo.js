    ...
    var Globalize = (function () {
        ...
        */
        Globalize.parseFloat = function (value, format) {
            ...
            // decimal
            var dp = wijmo.culture.Globalize.numberFormat['.'], rx = new RegExp('[^\\d\\' + dp + ']', 'g'), value = value.replace(rx, '').replace(dp, '.');
        ...
        */
        Globalize.parseDate = function (value, format) {
            ...
            var cal = wijmo.culture.Globalize.calendar, cjk = Globalize._CJK, rxv = new RegExp('(\\' + cal['/'] + ')|' + '(\\' + cal[':'] + ')|' + '(\\d+)|' + '([' + cjk + ']+)|' + '([' + cjk + '\\.]{2, })', 'gi'), vparts = value.match(rxv), fparts = Globalize._parseDateFormat(format), offset = 0, year = -1, month = 0, day = 1, hour = 0, min = 0, tzm = 0, sec = 0, ms = 0, era = -1;
    ...
    var Control = (function () {
        ...
        */
        Control.prototype.dispose = function () {
            ...
            // REVIEW: it would be nice to be able to remove all events that target this Control
            ...
            CollectionView.prototype._compareItems = function () {
                ...
                return function (a, b) {
                    for (var i = 0; i < sortDsc.length; i++) {
                        ...
                        //if (typeof (v1) === 'string') v1 = v1.toLowerCase();// + v1;
                        //if (typeof (v2) === 'string') v2 = v2.toLowerCase();// + v2;
                        // convert values
                        if (sortCvt) {
    ...
    var _MaskProvider = (function () {
        ...
        */
        ...
        _MaskProvider.prototype._keypress = function (e) {
            if (!e.ctrlKey && !e.metaKey && !e.altKey && this._preventKey(e.charCode)) {
                e.preventDefault();
            }
        };
        ...
        _MaskProvider.prototype._preventKey = function (charCode) {
            if (charCode && this._mskArr.length) {
                var input = this._tbx, start = input.selectionStart, key = String.fromCharCode(charCode);

                // make sure we're at a placeholder
                if (start < this._firstPos) {
                    start = this._firstPos;
                    wijmo.setSelectionRange(input, start);
                }
        ...
        _MaskProvider.prototype._connect = function (connect) {
            ...
            if (input) {
                if (connect) {
                    this._autoComplete = input.autocomplete;
                    this._spellCheck = input.spellcheck;
                    input.autocomplete = 'off'; // important for mobile browsers (including Chrome/Android)
                    input.spellcheck = false; // no spell-checking on masked inputs
                    input.addEventListener('input', this._hbInput);
                    input.addEventListener('keydown', this._hbKeyDown, true);
                    input.addEventListener('keypress', this._hbKeyPress, true);
                    this._valueChanged();
                } else {
                    input.autocomplete = this._autoComplete;
                    input.spellcheck = this._spellCheck;
                    input.removeEventListener('input', this._hbInput);
                    input.removeEventListener('keydown', this._hbKeyDown, true);
                    input.removeEventListener('keypress', this._hbKeyPress, true);
                }
            }
        };
        ...
        _MaskProvider.prototype._valueChanged = function () {
            ...
            if (input) {
                ...
                var start = input.selectionStart, oldChar = (start > 0) ? input.value[start - 1] : '';

                // apply the mask
                input.value = this._applyMask();

                // update the cursor if we have the focus
                if (document.activeElement == this._tbx) {
                    // backtrack if the original character was replaced with a prompt
                    var newChar = (start > 0) ? input.value[start - 1] : '';
